...
import {
    ...
    IdcardOutlined // 🌟 Thêm Icon đại diện cho Cấp bậc
...
const JobDetail = ({ isEmployer = false }) => {
    ...
    useEffect(() => {
        if (isApplyModalOpen && !isEmployer) {
            ...
            if (token) {
                ...
                const userId = decoded?.maUser || decoded?.nameid || decoded?.id || 1;
    ...
    const handleSendApplication = async () => {
        if (!agreed) return message.warning("Vui lòng đồng ý với Thỏa thuận sử dụng!");
        ...
        try {
            ...
            const res = await apiClient.post(`/Jobs/${id}/apply`, payload, {
                headers: { Authorization: `Bearer ${token}`, maUser: userId }
            });
            ...
            message.error(error.response?.data?.message || "Ứng tuyển thất bại do lỗi hệ thống!");
        }
    };
        ...
        <div style={{ background: '#141414', minHeight: '100vh', padding: '30px 20px', color: '#fff' }}>
            <div style={{ maxWidth: 1200, margin: '0 auto' }}>
                <Button 
                    ...
                    style={{ color: '#1890ff', marginBottom: 20 }}
                >
                            ...
                            job.danhSachViTri.map((viTri, index) => (
                                <Card
                                    ...
                                    extra={
                                        <Button
                                            type="primary"
                                            icon={isEmployer ? <FilterOutlined /> : <SendOutlined />}
                                            style={{
                                                backgroundColor: isEmployer ? '#1890ff' : '#fa8c16',
                                                borderColor: isEmployer ? '#1890ff' : '#fa8c16',
                                                fontWeight: 'bold'
                                            }}
                                            onClick={() => handleApplySpecificPosition(viTri.maViTri)}
                                        >
                                            {isEmployer ? 'Xem Phễu Ứng Viên' : 'Ứng tuyển ngay'}
                                        </Button>
                                    }
                                >
                                    {/* 🌟 THANH THÔNG SỐ VỊ TRÍ: HIỂN THỊ THÊM CẤP BẬC */}
                                    <div style={{
                                        marginBottom: 20, background: '#141414', padding: '10px 16px',
                                        borderRadius: '8px', border: '1px solid #262626', display: 'flex', flexWrap: 'wrap',
                                        ...
                                        {/* BỔ SUNG CẤP BẬC */}
                                        <Text style={{ color: '#13c2c2', fontWeight: '500', fontSize: '15px' }}>
                                            <IdcardOutlined /> Cấp bậc: {viTri.capBac || "Không yêu cầu"}
                                        </Text>

                                        <Text style={{ color: '#00b7c3', fontWeight: '500', fontSize: '15px' }}>
                                        ...
                                        <ul style={{ paddingLeft: 16, color: '#bfbfbf', margin: 0 }}>
                                            {job.danhSachViTri?.map(vt => (
                                                <li key={vt.maViTri} style={{ marginBottom: 8 }}>
                                                    ...
                                                    {/* 🌟 CẬP NHẬT CẤP BẬC VÀO SƠ LƯỢC VỊ TRÍ */}
                        ...
                        >
                            ...
                            <div style={{ marginTop: 24 }}>
                                ...
                                <Select className="custom-dark-select" popupClassName="custom-dark-dropdown" style={{ width: '100%' }} placeholder="-- Chọn vị trí công việc --" value={selectedViTri} onChange={(val) => setSelectedViTri(val)}>
                                    {job.danhSachViTri && job.danhSachViTri.length > 0 ? (
                                        job.danhSachViTri.map(vt => (
                                            /* 🌟 CẬP NHẬT CẤP BẬC VÀO DROPDOWN CHỌN VỊ TRÍ */
