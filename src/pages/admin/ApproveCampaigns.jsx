...
const ApproveCampaigns = () => {
    ...
    const fetchPendingCampaigns = async () => {
        ...
        try {
            ...
            const response = await axios.get('http://localhost:5279/api/AdminApproval/pending-campaigns', {
    ...
    const handleReview = async (id, isApproved) => {
        try {
            const token = localStorage.getItem('token');
            await axios.put(`http://localhost:5279/api/AdminApproval/review-campaign/${id}`, 
                { isApproved }, 
                { headers: { Authorization: `Bearer ${token}` } }
            );
